...
/**
 * Kafka consumer for category lifecycle events.
...
public class CategoryEventConsumer {
    ...
    @KafkaListener(
            topics = KafkaTopicNames.CATEGORY_EVENTS,
    ...
    public void consume(String message) {
        CategoryEvent event;
        try {
            event = objectMapper.readValue(message, CategoryEvent.class);
        } catch (JsonProcessingException ex) {
            log.error("Failed to deserialize CategoryEvent payload. Skipping invalid message: {}", ex.getMessage());
            return;
        }

        if (event.getEventId() == null) {
            log.warn("Received CategoryEvent with null eventId. Skipping.");
            return;
        }

        if (processedEventRepository.existsById(event.getEventId())) {
            log.info("Idempotent Consumer: CategoryEvent with eventId '{}' has already been processed. Skipping.",
                    event.getEventId());
            return;
        }
        ...
        try {
            log.info("Processing CategoryEvent: eventId={}, action={}, userId={}, aggregateId={}",
                    event.getEventId(), event.getAction(), event.getUserId(), event.getAggregateId());

            cacheEvictionService.evictUserCategoryCaches(event.getUserId());

            processedEventRepository.save(new ProcessedEvent(event.getEventId(), event.getEventType()));
            log.debug("CategoryEvent '{}' processed and marked in processed_events", event.getEventId());
        } finally {
            if (correlationId != null) {
